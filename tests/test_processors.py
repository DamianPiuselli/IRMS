...
def test_load_data_renaming(mock_read_excel):
    ...
    # Note: 'Ampl  28' has double spaces to test whitespace normalization
    raw_data = {
...
def test_peak_filtering_default(mock_read_excel):
    ...
    # Setup: Peak 1 (Ref), Peak 2 (Sample)
    raw_data = {
        "Identifier 1": ["S1", "S1"],
        "Peak Nr": [1, 2],
        "d 15N/14N": [0.0, 10.0],
...
def test_exclude_rows(mock_read_excel):
    """Test that specific rows can be excluded (e.g. bad injections)."""
    raw_data = {
        "Identifier 1": ["S1", "S2"],
        "Peak Nr": [2, 2],
        "d 15N/14N": [10.0, 12.0],
